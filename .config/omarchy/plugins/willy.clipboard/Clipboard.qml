...
Item {
  ...
  property int rowHeight: Math.max(Style.space(50), root.fs(Style.font.body) + root.fs(Style.font.caption) + Style.spacing.rowPaddingX * 2)
  ...
  function applySelected(row) {
    ...
    if (row.entryType === "image") {
      Quickshell.execDetached([Quickshell.env("HOME") + "/.local/bin/willy-clipboard-paste-file", row.mime, row.path])
    } else if (row.fullText) {
      Quickshell.execDetached([Quickshell.env("HOME") + "/.local/bin/willy-clipboard-paste-text", "--shift-insert", "--history-index", String(row.historyIndex)])
    }
  }
  ...
  function copySelected(row) {
    ...
    root.opened = false
    if (row.entryType === "image") {
      Quickshell.execDetached([Quickshell.env("HOME") + "/.local/bin/willy-clipboard-paste-file", "--copy-only", row.mime, row.path])
    } else if (row.fullText) {
      Quickshell.execDetached([Quickshell.env("HOME") + "/.local/bin/willy-clipboard-paste-text", "--copy-only", "--history-index", String(row.historyIndex)])
    }
  }
              ...
              ListView {
                ...
                delegate: Rectangle {
                  ...
                  Row {
                    ...
                    Image {
                      readonly property int thumb: root.rowHeight - Style.space(16)
                      visible: parent.parent.previewImage.length > 0
                      width: visible ? thumb : 0
                      height: thumb
                      sourceSize.height: thumb
